...
public class AdminPageServiceTest {
    ...
    public void setUp() throws ReadClientException, ReadCardException {
        ...
        Mockito.when(clientDao.getClients()).thenReturn(clientList);
        Mockito.when(cardDao.getCardsToUnblock()).thenReturn(cardList.stream().filter(x->x.getCardStatus().getId() == 3 )
    ...
    public void testGetClients() throws ReadClientException {
        List<Client> testClients = adminPageService.getClients();
    ...
    public void testGetCards() throws ReadCardException {
        List<Card> testCards = adminPageService.getCards();
